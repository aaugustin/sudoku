...
class TestGrid(unittest.TestCase):
    ...
    def test_from_string(self):
        ...
        for input, expected in tests:
            with self.subTest(input=input):
                self.assertEqual(Grid.from_string(input), expected)
    ...
    def test_to_string(self):
        ...
        # fmt: on
        for grid, format, expected in tests:
            with self.subTest(grid=grid, format=format):
                self.assertEqual(grid.to_string(format), expected)

    def test_to_string_error(self):
        grid = Grid()
        with self.assertRaises(ValueError) as raised:
            grid.to_string("3d")
        self.assertEqual(str(raised.exception), "unsupported format: 3d")
