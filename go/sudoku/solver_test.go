...
func TestSolve(t *testing.T) {
	...
	for _, test := range tests {
		...
		for i, g := range gotSolutions {
			w := test.wantSolutions[i]
			if g != w {
				t.Errorf("%v: #%d: got %v, want %v", test.input, i, g, w)
			}
		}
		if math.Abs(gotDifficulty-test.wantDifficulty) > 0.01 {
			t.Errorf("%v: expected difficulty %.2f, got %.2f", test.input, test.wantDifficulty, gotDifficulty)
		}
	}
}
